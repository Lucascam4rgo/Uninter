...
create table if not exists locacao (
    ...
    constraint fkCliente1 foreign key (idCliente)
    references cliente(idCliente),
	constraint fkPagamento1 foreign key (idPagamento)
...
create table if not exists manutencao (
    ...
    constraint fkVeiculo1 foreign key (idVeiculo)
...
create table if not exists locacaoVeiculo (
    ...
    idVeiculo int not null,
    constraint fkVeiculo2 foreign key (idVeiculo)
    references veiculo(idVeiculo),
    constraint fkLocacao1 foreign key (idLocacao)
    references locacao(idLocacao)
);

select descricao, dataManutencao, custo from manutencao;

select sum(valorTotal) as 'Valor arrecadado' 
from pagamento
where estado != 'Pendente';

select veiculo.modelo as 'Modelo', veiculo.marca as 'Marca', count(numeroLocacoes.idVeiculo) as 'Número de Aluguéis' from veiculo
inner join locacaoVeiculo as numeroLocacoes
on veiculo.idVeiculo = numeroLocacoes.idVeiculo
group by veiculo.modelo, veiculo.marca
order by 'Numero de Aluguéis' desc;
;
...
select cliente.nome as 'Nome do Cliente',
	pagamento.estado as 'Estado',
	sum(pagamento.valorTotal) as 'Valor Pendente'
from cliente
...
/* select concat('alter table ', table_name, ' modify column ', column_name, ' ', column_type, ' not null;')
from information_schema.columns
where table_schema = 'LocadoraVeiculos'
	and is_nullable = 'YES';
